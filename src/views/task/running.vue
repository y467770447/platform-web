      ...
      <div class="table-box">
        <el-table :data="state.tableData" style="width: 100%; height: 100%">
          ...
          <el-table-column label="操作" fixed="right">
            <template #default="{ row }">
              <el-link type="primary" :underline="false" @click="stoptaskRow(row)">停止运行</el-link>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { stoptask, taskList } from "@/api/task/index.js"
...
const stoptaskRow = (row) => {
  ElMessageBox.confirm("确认停止运行?", "提示", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    type: "warning"
  })
    .then(() => {
      stoptask({ ids: row.id }).then(() => {
        gettaskList()
      })
    })
    .catch(() => {
