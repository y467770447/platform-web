        ...
        <div class="write-in-box">
          ...
          <div class="table-box">
            ...
            >
              ...
              <el-table-column property="colId" label="输入字段id">
              ...
              <el-table-column property="symbol" label="比较运算符">
                <template #default="{ row }">
              ...
              <el-table-column property="colValue" label="输入字段比较值">
                <template #default="{ row }">
                  <div v-show="!row.isEdit">{{ row.colValue }}</div>
                  <el-input v-show="row.isEdit" v-model="row.colValue" @change="changeTableData" />
                </template>
              </el-table-column>
              <el-table-column property="targetValue" label="替换为新值">
                <template #default="{ row }">
                  <div v-show="!row.isEdit">{{ row.targetValue }}</div>
                  <el-input
                    v-show="row.isEdit"
                    v-model="row.targetValue"
                    @change="changeTableData"
                    placeholder="${字段id}获取字段值以及变量"
                  />
                </template>
              </el-table-column>
              <el-table-column property="newColId" label="输出字段id">
                <template #default="{ row }">
                  <div v-show="!row.isEdit">{{ row.newColId }}</div>
                  <el-input
                    v-show="row.isEdit"
                    v-model="row.newColId"
                    @change="changeTableData"
                    placeholder="新值赋给本字段"
                  />
                </template>
...
import { nodeDetail, writeInSave, sjszjdList, columnList } from "@/api/dataView/lcsj.js"
